...
/// Окно TLI: обратный отсчёт до момента, когда орбита позволяет начать burn.
#[derive(Resource, Default, Debug)]
pub struct TliWindow {
    /// Секунды до открытия окна (0 = можно жечь).
    pub countdown_s: f32,
...
pub fn plugin(app: &mut App) {
    app.init_resource::<TliBurnState>()
        ...
        .init_resource::<TliWindow>()
        .add_systems(OnEnter(MissionStage::TLI), setup_tli_scene)
        .add_systems(
            Update,
            (tick_tli_window, handle_burn_input, tick_burn)
                .chain()
                .run_if(in_state(MissionStage::TLI)),
        );
}
...
fn tick_tli_window(
    ...
    window.countdown_s = (window.countdown_s - dt).max(0.0);
    if window.countdown_s <= 0.0 {
        window.window_open = true;
        info!("stages/tli: окно TLI открыто — можно начинать burn");
    }
}
...
fn handle_burn_input(
    ...
    if *burn_state == TliBurnState::Idle && window.window_open && keys.just_pressed(KeyCode::Space) {
